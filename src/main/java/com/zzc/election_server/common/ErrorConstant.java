 ...
 * @time 2019/1/23 17:24
 */
public class ErrorConstant {
    /**
     *
     */
    private static Integer errMessageEndIndex = 50;
    ...
    public static final String HSF_CLASS_CHECK = "com.taobao.hsf.exception.HSFServiceAddressNotFoundException";

    public static final String HSF_CLASS_CHECK_MESSAGE = "服务未启动";
     ...
     * 抛出Error
     */
    public static void getErrorException(String errCode) throws BusinessException {
        if (!SUCCESS.equals(errCode)) {
            if (errorResultMap.containsKey(errCode)) {
                throw new BusinessException(errorResultMap.get(errCode).getText());
            } else {
                throw new BusinessException(errorResultMap.get(UNKNOW_ERROR).getText());
            }
        }
    }
     ...
     * 抛出Error
     */
    public static Result getErrorException(Exception e, String text) throws BusinessException {
     ...
     * 抛出Error
     */
    public static Result getErrorException(Exception e, String errCode, String text) throws BusinessException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        e.printStackTrace(new PrintStream(baos));
        Result result = new Result();
        result.setStatus(ErrorConstant.UNKNOW_ERROR);
    ...
    public static void getErrorException(String errCode, String errorMessage) {
        if (!SUCCESS.equals(errCode)) {
            throw new BusinessException(errorMessage, errCode);
        }
    }
     ...
     * HSF服务调用异常
     */
    public static boolean checkHsfError(Result result) {
        if (result != null && HSF_SERVICE_ERROR.equals(result.getStatus())) {
            return true;
        }
        return false;
    }
...
//        Result r = errorResultMap.get(SUCCESS);
        Result r = new Result();
        r.setT(t);
